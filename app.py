...
whatsapp_message = quote(
    f"Check out Brian Mtepe's Clinical Data Portfolio: {live_url}"
)
...
with st.sidebar:
    ...
    st.markdown(
        f'<a href="https://wa.me/?text={whatsapp_message}" class="nav-card">📲 Share via WhatsApp</a>',
        unsafe_allow_html=True
    )
    ...
    st.success(
        "👨‍⚕️ Specialist: MCH Dashboards & SQL"
    )
...
st.markdown(
    ...
    unsafe_allow_html=True
)nter;opacity:0.6;'>Healthcare Portfolio | Remote Global Availability | 2026</div>",
    unsafe_allow_html=True
)============================
st.markdown("---")
st.markdown(
