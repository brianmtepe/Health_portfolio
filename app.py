...
# =========================================================
# DATA (CONSOLIDATED ACTIVE PROJECTS)
# =========================================================
projects = [
...
with st.sidebar:
    ...
    st.markdown(f'<a href="https://wa.me/?text={whatsapp_message}" class="nav-card">📲 Share via WhatsApp</a>', unsafe_allow_html=True)
    st.divider()
...
with col_right:
    ...
    st.markdown('<div class="tech-pill">Python | SQL | Power BI | DHIS2</div>', unsafe_allow_html=True)
...
p_cols = st.columns(3)
for idx, (project_name, description) in enumerate(projects):
    with p_cols[idx]:
        st.markdown(f"""<div class="tech-box"><b>{project_name}</b><br><small>{description}</small></div>""", unsafe_allow_html=True)
